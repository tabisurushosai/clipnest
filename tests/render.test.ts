...
import { truncatePreview } from '../src/popup/render';
...
describe('render helpers', () => {
  ...
  it('sorts single clip', () => {
    ...
    expect(sortClips([clip]).map((c) => c.id)).toEqual(['a']);
  });
  ...
  it('sorts ten clips by created_at desc when unpinned', () => {
    ...
    const sorted = sortClips(clips, 'newest');
  ...
  it('places pinned clips before unpinned regardless of created_at', () => {
    ...
    const sorted = sortClips([fresh, pinnedOld], 'newest');
